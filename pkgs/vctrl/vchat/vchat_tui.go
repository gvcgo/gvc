...
func InitialModel(chatgpt *VChat, conversations *ConvManager) model {
	ta := textarea.New()
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case key.Matches(msg, m.keymap.NewConversation):
			...
			m.conversations.New(m.conversations.Conf.Conversation)
			m.viewport.SetContent(m.RenderConversation(m.viewport.Width))
			m.viewport.GotoBottom()
			m.historyIdx = 0
	...
	case deltaAnswerMsg:
		...
		cmds = append(cmds, m.chatgpt.Receive())
		m.err = nil
		m.viewport.SetContent(m.RenderConversation(m.viewport.Width))
		m.viewport.GotoBottom()
	case answerMsg:
...
func (m model) RenderConversation(maxWidth int) string {
	...
	renderYou := func(content string) {
		...
		sb.WriteString(utils.EnsureTrailingNewline(content))
	}
